...
// // In-memory store
// const polls = {}; // pollId -> {title, students: {socketId: name}, current: {...}, history: [], timeout }

// function makeId() {
//   return Math.random().toString(36).slice(2,9);
// }

// app.get('/', (req, res) => res.send('Live Poll Server OK'));
...
//     const pollId = makeId();
//     polls[pollId] = { title: title || 'Untitled Poll', students: {}, current: null, history: [] };
...
//   // ✅ Make teacher socket join the room on backend
//   socket.join(pollId);
...
//   // ✅ Broadcast question to ALL sockets in this poll
...
//       options: current.options 
//     });
//     polls[pollId].history.push({ ...polls[pollId].current, results: res });
//     polls[pollId].current = null;
//     delete polls[pollId].timeout;
//   }, duration * 1000);
...
//   cb && cb({ok:true, students: Object.values(polls[pollId].students), pollTitle: polls[pollId].title});
// });
...
//     if (poll.current.answers[socket.id] !== undefined) return cb && cb({ok:false, error:'Already answered'});
...
//     const totalAnswered = Object.keys(poll.current.answers).length;
//     if (totalStudents > 0 && totalAnswered >= totalStudents) {
//       if (poll.timeout) { clearTimeout(poll.timeout); delete poll.timeout; }
//       const res = computeResults(poll.current);
//       poll.history.push({ ...poll.current, results: res });
//       io.to(pollId).emit('question:end', { results: res, correctIndex: current.correctIndex, options: current.options });
...
//   socket.on('teacher:get-history', ({pollId}, cb) => {
//     if (!polls[pollId]) return cb && cb({ok:false, error:'Poll not found'});
//     cb && cb({ok:true, history: polls[pollId].history});
//   });

//   socket.on('disconnect', () => {
//     for (const [pid, poll] of Object.entries(polls)) {
//       if (poll.students && poll.students[socket.id]) {
